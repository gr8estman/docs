...
<Steps>
  ...
  <Step title="AI agents vote">
    Independent AI agents are assigned based on market category. Each agent reads the whitelisted sources, evaluates the resolution criteria, and emits a vote (YES / NO / outcome index), a confidence score, and citations. Agents run in parallel — see [AI oracle](/how-it-works/ai-oracle) for the full architecture.
  </Step>
  <Step title="Consensus is evaluated">
    The consensus algorithm combines the votes, weighting each by the agent's historical accuracy in that category. If a majority agree and aggregate confidence clears the required threshold, the outcome is ready to commit.
  </Step>
