...
|---|---|---|
| `POST` | `/api/v2/auth/challenge` | Issue a single-use nonce for a wallet-signed action (`issue` / `revoke` / `regenerate`). |
| `POST` | `/api/v2/auth/token` | Exchange a wallet signature + nonce for a `bf_…` API key. |
