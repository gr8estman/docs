...
<AccordionGroup>
  ...
  <Accordion title="Time-decaying impact cap">
    The maximum price impact per trade tightens as resolution approaches. Large orders placed close to resolution receive a worse fill rather than moving the price significantly.
  </Accordion>
  <Accordion title="Position cap">
    Per-wallet USDC exposure on a single round steps down as the round nears resolution, which prevents late-stage position-building by well-capitalized accounts. The current schedule is shown on the market page.
  </Accordion>
...
The standard creator fee is **0.5%** per trade, plus a small protocol fee. As a round approaches resolution, the effective fee ramps up to deter last-second informed flow. This ramp applies only in the window immediately before the halt — the live fee for any round is shown on the market page before you confirm a trade.
...
<AccordionGroup>
  ...
  <Accordion title="What if the Pyth feed is unavailable?">
    The engine falls through automatically: **Pyth Hermes → CoinGecko → Binance**. If all three sources are unavailable at the resolution moment, the round is voided and all traders are refunded in full.
  </Accordion>
