...
Kalshi is explicitly legal under CFTC oversight and treats event contracts as regulated derivatives. Polymarket geo-blocks US users via its Terms of Service. BlockForecast similarly restricts users in jurisdictions where operating would be legally problematic, and currently does not list US-political markets to avoid the specific regulatory gray area. **Each user is responsible for compliance with their own state's laws.**
...
- Does not currently require KYC for standard trading, but may add it for specific markets or jurisdictions if regulation demands it.
- Does not currently list US-political markets.
